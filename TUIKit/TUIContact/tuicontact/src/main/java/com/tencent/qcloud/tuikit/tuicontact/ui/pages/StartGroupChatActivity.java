...
public class StartGroupChatActivity extends BaseLightActivity {
    ...
    private void init() {
        ...
        mTitleBar.setOnRightClickListener(v -> createGroupChat());
        mTitleBar.setOnLeftClickListener(v -> finish());

        mJoinType = findViewById(R.id.group_type_join);
        mJoinType.setOnClickListener(v -> showJoinTypePickerView());
        ...
        mContactListView.setOnSelectChangeListener((contact, selected) -> {
            if (selected) {
                GroupMemberInfo memberInfo1 = new GroupMemberInfo();
                memberInfo1.setAccount(contact.getId());
                mMembers.add(memberInfo1);
            } else {
    ...
    private void showNotSupportDialog() {
        ...
        TUIKitDialog.TUIIMUpdateDialog.getInstance()
                ...
                .setPositiveButton(getString(R.string.contact_no_more_reminders), v -> {
                    TUIKitDialog.TUIIMUpdateDialog.getInstance().dismiss();
                    TUIKitDialog.TUIIMUpdateDialog.getInstance().setNeverShow(true);
                })
                .setNegativeButton(getString(R.string.contact_i_know), v -> TUIKitDialog.TUIIMUpdateDialog.getInstance().dismiss())
                .show();
    }
